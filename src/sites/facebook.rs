...
impl crate::sites::Site for Facebook {
    ...
    fn user(&self, _: &elephantry::Pool, id: &str, _: &str) -> crate::Result<crate::sites::User> {
        ...
        let mut user = crate::sites::User {
            ...
            description: self.og(&html, "description").ok(),
            url: self.og(&html, "url").unwrap_or(url),
            image: self.og(&html, "image").ok(),
            posts: vec![],
        };

        let id_regex = regex::Regex::new("&id=([^&]+)").unwrap();
        ...
        for element in self.select(&root, "div[data-ft]") {
            ...
            let url = match self
                .select_first(&element, "div:last-child > div:last-child > a:last-child")
            {
...
impl Facebook {
    fn rewrite_href(&self, contents: &str) -> String {
        ...
        let contents = regex
            .replace_all(contents, r#"href="https://mobile.facebook.com$1""#)
            .to_string();
        ...
        regex
            .replace_all(&contents, r#"href="/post/facebook/$1-$2"#)
            .to_string()
    }
    ...
    fn parse_date(text: &str) -> String {
        let regex = regex::Regex::new("^(\\d+) hrs$").unwrap();
